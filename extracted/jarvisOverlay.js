...
(function () {
  "use strict";
   ...
   function showFatalError(msg) {
     try {
       var box = document.getElementById("lp-jarvis-fatal");
       if (!box) {
         box = document.createElement("div");
         box.id = "lp-jarvis-fatal";
         box.style.cssText = "position:fixed;inset:0;padding:16px;color:#ffb4b4;" +
           "font:12px/1.5 monospace;white-space:pre-wrap;overflow:auto;z-index:99999;" +
  ...
  // Jika skrip ini dimuat sebagai CONTENT SCRIPT pada halaman sidebar host
  // (jarvisSidebar.html), jANGAN jalankan di sini — biarkan instance yang dimuat
  // oleh <script src> pada halaman itu (yang berkongsi window yang sama) yang
  // membina panel. Jika tidak, instance content script akan set
  // window.__lpJarvisInstalled lebih awal dan menyebabkan instance halaman
  // berhenti (return) → panel sidebar kekal KOSONG.
...
function lpStorageGet(keys, cb) {
  ...
  var selSearchEngines = []; // ids enabled (from settings)
  var selSearchEnabled = false; // master toggle for selection search
  ...
  function ensureSelButtons() {
    ...
    var aiBtn = el("button", { className: "lp-jarvis-sel-ai", text: "Ai", title: "Hantar teks pilihan ke AI" });
    var noteBtn = el("button", { className: "lp-jarvis-sel-note", text: "＋ Note", title: "Masukkan teks pilihan ke nota" });
    selButtonsEl.appendChild(aiBtn);
    selButtonsEl.appendChild(noteBtn);
    // Selection-search engine buttons (skipped entirely when disabled).
    if (selSearchEnabled) {
      for (var i = 0; i < SEARCH_ENGINES.length; i++) {
        (function (eng) {
          if (selSearchEngines.indexOf(eng.id) === -1) return;
          var eb = el("button", { className: "lp-jarvis-sel-search", text: eng.label, title: eng.title });
          eb.addEventListener("mousedown", function (e) { e.preventDefault(); e.stopPropagation(); });
          eb.addEventListener("click", function (e) {
            e.preventDefault(); e.stopPropagation();
            var txt = getSelectionText();
            hideSelButtons();
            if (txt) { try { window.open(buildSelectionSearchUrl(eng.id, txt), "_blank"); } catch (e2) {} }
          });
          selButtonsEl.appendChild(eb);
        })(SEARCH_ENGINES[i]);
      }
    }
    (document.body || document.documentElement).appendChild(selButtonsEl);
    aiBtn.addEventListener("mousedown", function (e) { e.preventDefault(); e.stopPropagation(); });
    ...
    noteBtn.addEventListener("mousedown", function (e) { e.preventDefault(); e.stopPropagation(); });
    noteBtn.addEventListener("click", function (e) {
      ...
      if (txt) {
        ...
        addBubble("jarvis", "Teks pilihan dihantar ke nota Local Pocket.");
      }
    });
  ...
  var regionRect = null;
  var _pendingHostCaptureRegionCallback = null; // used when sidebar relays region capture to the active tab
  var _pendingHostCaptureImageCallback = null;  // used when sidebar relays image select to the active tab
  ...
  function buildPanel() {
   try {
    ...
    // and the action row (attach / upload / textarea / send) at the bottom.
    ...
    var inputBottom = el("div", { className: "lp-jarvis-input-bottom" }, [imgAttachBtn, fileUploadBtn, inputEl, bottomToggleBtn, sendBtn]);
    ...
    var QUICK_CHIPS = [
      ...
      { label: "⏱ Pomodoro", cmd: "buka pomodoro" }
    ];
    ...
    var bottomSection = el("div", { className: "lp-jarvis-bottom-section" }, [memoryRow, fontRow, hint]);
    ...
    // Floating "back to JARVIS" control, painted above the provider iframe so it
    ...
    // Posisi tengah sebaik panel dibina (sebelum visible) — tiada kebergantungan
    // pada async storage, jadi panel terus muncul di tengah tanpa flash.
    centerPanel();
    ...
    if (modeToggleBtn) modeToggleBtn.addEventListener("click", function (e) {
      e.stopPropagation();
      // FLIP: kita sedang dalam OVERLAY → tutup overlay dan buka TERUS sidebar
      // JARVIS (dan sekalikan default buka JARVIS = sidebar). Begitu juga sebaliknya
      // dikendalikan dalam jarvisSidebar.js (SIDEBAR_HOST).
      try {
        setJarvisOpenMode("sidebar");
        if (open) { try { closePanelIfOpen(); } catch (e2) {} }
        try { api.runtime.sendMessage({ type: "open-jarvis-by-mode", mode: "sidebar" }); } catch (e3) {}
      } catch (e0) {}
    });
    updateModeToggleLabel();
    ...
    function resolveImageFromDrop(dt, cb) {
      ...
      var url = "";
      // Prefer the actual <img src> from text/html (Google Images etc. wrap
      // the real picture in an iframe/overlay — text/uri-list gives the
      // intermediary page URL, not the image).
      try {
    ...
    if (_imgDropTarget) {
      ...
      _imgDropTarget.addEventListener("drop", function (e) {
        ...
        // handler is the canonical, robust one (it converts web-page image
        // URLs to data URLs via the background). Letting the event bubble
        // to it guarantees the picture is captured even for drops that land
    ...
    var SUGGEST_DEFAULTS = [
      ...
      "buka notes", "pomodoro 10", "buka settings", "salin jawapan", "cetak halaman"
    ];
  ...
  function registerOutsideClose() {
    ...
    outsideCloseAttached = true;
     document.addEventListener("click", function (e) {
      ...
      // Jangan tutup bila klik pada UI sambungan sendiri (butang "Ai" pilihan
      // teks, bekas floating, dsb) — klik tersebut bukan niat tutup panel.
      if (e.target && e.target.closest) {
        if (e.target.closest("#__lp_ai_selection_container, #__pocket_container, #lp-jarvis-root")) return;
      }
      toggle();
    }, true);
  }
  ...
  function centerPanel() {
    if (!root) return;
    // Guna saiz CSS sebenar (walaupun panel display:none, kira guna
    // nilai min() yang setara supaya posisi tepat tanpa perlu visible).
    var cssW = Math.min(480, window.innerWidth - 24);
    var cssH = Math.min(640, window.innerHeight - 36);
    // Jika panel sudah visible, ukur offset sebenar (mungkin berbeza
    // selepas resize). Jika masih display:none, guna CSS default.
    var w = root.offsetWidth > 0 ? root.offsetWidth : cssW;
    var h = root.offsetHeight > 0 ? root.offsetHeight : cssH;
    var left = Math.max(PANEL_MARGIN, Math.round((window.innerWidth - w) / 2));
    var top = Math.max(PANEL_MARGIN, Math.round((window.innerHeight - h) / 2));
    root.style.left = left + "px";
    root.style.top = top + "px";
    root.style.right = "auto";
    root.style.bottom = "auto";
  ...
  function initResize(handle) {
    ...
    // If none exists, center the panel for first-time use.
    try {
   ...
   function toggleProviderView() {
     ...
     if (providerVisible) {
      ...
      // When the AI view is hidden, release the provider session so it doesn't
      // compete with the Local Pocket AI Sidebar's own provider.
      if (!providerVisible) destroyProviderIframe();
    ...
    // When hidden we keep it inert (property + attribute) so Gemini's composer
    // can't steal the cursor; CSS (opacity:0; pointer-events:none) + the
  ...
  function stopFocusKeeper() {
   ...
   function startFocusKeeper() {
     ...
     focusKeeperTimer = setInterval(function () {
       ...
       if (providerVisible) return;
       if (isPanelControl(ae)) return;
       // If focus is anywhere outside the panel (body, document, or any
       // non-interactive element), politely pull it back to the JARVIS
       // input so cursor always stays in the chat box without F6.
       var now = Date.now();
  ...
  function togglePin() {
    ...
    function toggle() {
      ...
      if (open) {
         ...
         applyJarvisModeClass();
         // Tolak tetingkap "close outside" supaya klik yang MEMBUKA panel
         // (cth. butang "Ai" pada overlay pilihan teks) tidak terus menutup
         // JARVIS sebaik sahaja ia dibuka. Ini elakkan panel tertutup serta-merta
         // bila ia dibuka melalui UI sambungan sendiri.
        ...
        // Local Pocket AI Sidebar's own provider after JARVIS is closed.
        destroyProviderIframe();
      }
   }
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            if (isImage) {
              ...
              reader.onload = function (ev) {
                try { pendingImage = ev.target.result; pendingUserImage = ev.target.result; showPendingImageThumb(true); } catch (e2) {}
              };
  ...
  function setBubbleText(bubble, text) {
   ...
   function renderBubble(role, text, imgDataUrl, ts) {
      ...
      return bubble;
      } catch (e) { console.error("[JARVIS renderBubble]", e); return null; }
    }

   function addBubble(role, text, imgDataUrl) {
      try {
      // A user bubble rendered without an explicit image must not inherit a stale
      // pending image from an earlier upload/screenshot that was never sent.
      if (role === "user" && !imgDataUrl) pendingUserImage = null;
      ...
      return b;
      } catch (e) { console.error("[JARVIS addBubble]", e); return null; }
    }

   // Render a user bubble that also shows the image attached to this question
   // (Gemini-style thumbnail inside the chat bubble). Consumes the pending
   // user image so it can't bleed into a later, unrelated turn.
  ...
  function showCapabilities() {
    var lines = [
      ...
      "• Add-on — \"buka sidebar\", \"buka notes\", \"pomodoro 10\", \"buka settings\"",
    ...
    // bypassing linkify's plain-text escaping. The sanitize happens in the
    // coalesced frame flush / final render (NOT per chunk).
   ...
   */
   ...
   var SLASH_COMMANDS = {
    ...
    function handleSlashCommand(text, done) {
     ...
     switch (cmd) {
       ...
       case "/theme":     addBubble("jarvis", "Tukar tema dalam panel Tetapan."); done(); return true;
       case "/session":   addBubble("jarvis", "Gunakan panel Session untuk mengurus sesi."); done(); return true;
   ...
   function processOne(text, done) {
     ...
     if (mc === "export") { addBubble("user", text); exportMemory(); return done(); }
     if (mc === "import") { addBubble("user", text); importMemoryViaPicker(); return done(); }
     ...
     if (/^(simpan|save)\s+perbualan|^save\s+conversation/.test(lowerCmd)) {
       addBubble("user", text); doSaveConversation(); return done();
     }
      if (/^(senarai|list)\s+perbualan|^list\s+conversations/.test(lowerCmd)) {
        addBubble("user", text); listConversations(); return done();
      }
...
return;
       ...
       // help / self_check / export_diagnostic perlu done callback
       if (pre.type === "help") { addUserBubble(text); showCapabilities(); return done(); }
       if (pre.type === "self_check") { addBubble("user", text); runSelfCheck(done); return; }
       if (pre.type === "export_diagnostic") { addBubble("user", text); exportDiagnostic(done); return; }
        // search + explicit scope → local
        ...
        // search + ambiguous / youtube → Gemini (via cariAskAI)
        if (pre.type === "search" && (pre.ambiguous || pre.mode === "youtube")) {
          cariAskAI(text, pre, done);
          return;
        }
        // "buka" dengan URL valid → browser action local
        // "buka X" (bukan URL) → Gemini (cariAskAI) — jangan web search / teka .com
       ...
       if (macroMatch) { addBubble("user", text); enqueueMacro(macroMatch.name, macroMatch.commands); return done(); }
       var cached = recallCommand(text);
       if (cached && cached.plan) {
         addBubble("user", text); bumpLearnedHit(cached);
  ...
  function runSinglePlan(plan, userText, done) {
    ...
    switch (action) {
      ...
      case "click_first_link": doClickFirstLink(callDone, plan.target); return;
    ...
    // "buka youtube dan klik link pertama"). This makes the chained click fire
   ...
   // ─────────────────────────────────────────────────────────────────────
   // Conflict Resolution UI (Fasa 1)
   // ─────────────────────────────────────────────────────────────────────

   /**
    * Render conflict alternatives as selectable cards in the transcript.
    * @param {Array} conflicts  Output of Core.detectConflicts()
    * @param {Function} onResolve  Called with the chosen { conflict, alternative }
    */
   function renderConflictUI(conflicts, onResolve) {
     if (!transcriptEl) buildPanel();
     if (!conflicts || !conflicts.length) return;
     // Show each conflict as a card group
   ...
   /**
    * Render a single conflict with its alternatives as visual cards.
    */
   function renderConflictCard(conflict, onResolve) {
     ...
     // Alternatives as clickable cards
     ...
     conflict.alternatives.forEach(function (alt) {
       var card = el("button", {
         className: "lp-jarvis-conflict-alt",
         type: "button"
       });
       // Icon area
       var iconMap = {
       ...
       if (conflict.allow_edit && alt.plan) {
         ...
         editBtn.addEventListener("click", function (e) {
           e.stopPropagation();
           editAlternative(alt, conflict, function (modified) {
             pick(modified);
           });
         });
     ...
     if (conflict.allow_custom) {
       ...
       customCard.addEventListener("click", function () {
         customInputModal(conflict, function (alt) {
           if (alt) pick(alt);
         });
       });
     ...
     function onKey(e) {
       ...
       if (k === "ArrowRight" || k === "ArrowDown") {
         ...
         e.preventDefault(); e.stopPropagation();
         if (altButtons[focusIdx]) altButtons[focusIdx].click();
       } else if (k === "Escape") {
         e.preventDefault(); e.stopPropagation();
         pick(null);
       }
     ...
     var timerId = setTimeout(function () {
       if (!picked) {
         pick(null);
         addBubble("jarvis", "Masa tamat — dibatalkan.");
       }
     }, timeoutMs);

     // "Jangan tanya lagi" handler
     var cb = dontAsk.querySelector("input");
     cb.addEventListener("change", function () {
       if (cb.checked) {
         try {
           var prefs = {};
           prefs["jarvisConflictPrefs"] = {};
           prefs["jarvisConflictPrefs"][conflict.type] = "always_ask";
           api.storage.local.set(prefs).catch(function () {});
         } catch (e) {}
       }
     });
     ...
     // Clean up timer + keydown when removed
   ...
   /**
    * Open an inline edit modal to modify an alternative before execution.
    */
   function editAlternative(alt, conflict, onModified) {
     ...
     saveBtn.addEventListener("click", function () {
       ...
       inputs.forEach(function (inp) {
         ...
         if (key && key !== "action") modified[key] = inp.value;
       });
       backdrop.remove();
   ...
   /** Build a labeled input field for the edit modal. */
   function buildField(label, type, value, readonly) {
     ...
     var input = el("input", {
       type: type,
       className: "lp-jarvis-modal-field-input",
       value: value,
   ...
   /**
    * Open a modal for custom input when user picks "Taip sendiri".
    */
   function customInputModal(conflict, onResult) {
     ...
     var textarea = el("textarea", {
       className: "lp-jarvis-modal-textarea",
       placeholder: "Contoh: cari resepi nasi lemak di YouTube"
     });
     ...
     function sendCustom() {
       ...
       backdrop.remove();
       onResult({ id: "custom", label: "Custom", description: t, preview: t, plan: null, customText: t });
     }
   ...
   /**
    * Resolve a conflict: execute the chosen plan with memory.
    */
   function resolveConflict(chosenAlt, userText) {
     if (!chosenAlt || !chosenAlt.plan) {
       addBubble("jarvis", "Dibatalkan.");
       focusInput();
       return;
     }
     ...
     if (Array.isArray(plan)) {
       runPlanSequential(plan, 0, userText || "", 0);
     } else {
   ...
   // re-plan before the rest of the plan continues.
             ...
             po = new PerformanceObserver(function (list) {
               ...
               for (var i = 0; i < entries.length; i++) {
                 ...
                 if (e.entryType === "resource") {
                   // responseEnd === 0 means the request is still in flight.
     ...
     function runPlanSequential(plans, idx, userText, depth, onComplete) {
      ...
      // commands). On failure we skip the step and keep going.
         function step() {
          ...
          // the LOADED page, not on the pre-navigation DOM.
          if (act === "fill" && next) {
            var fillRes = runFillPlan(plan);
            if (fillRes && fillRes.searchNavigate) {
              // YouTube search: navigate via the background so the remaining plan
              // is armed reliably on the results page (same proven `navigate` +
              // pendingPlan mechanism used for the first navigation step).
              sendBrowser("navigate", { url: fillRes.searchNavigate, pendingPlan: plans.slice(idx + 1), userText: userText || "" });
            } else {
              // Generic fill that may submit & navigate: arm the rest on pagehide.
     ...
     // Run a `fill` plan step and return its result (so the caller can detect a
     // search navigation and hand off the rest of the plan reliably).
     function runFillPlan(plan) {
       try {
         return doFill(str(plan.field), str(plan.value));
       } catch (e) {
    ...
    switch (intent.type) {
      ...
      case "navigate": doNavigate(intent.target); break;
        case "click_first_link": doClickFirstLink(done, intent.target); break;
  ...
  // in favour of the first genuine content link.
   function doClickFirstLink(done, target) {
    ...
    var firstTested = false;
    function firstLink() {
      var firstReal = null;   // first link with a genuinely navigable href
      var firstText = null;   // first such link that also has visible text
      var firstMatch = null;   // first such link whose text contains the target word
      try {
        walkAll(document.documentElement, function (node) {
          if (firstMatch) return; // already have the best candidate
          if (!node.tagName || node.tagName.toLowerCase() !== "a") return;
          if (!isVisible(node)) return;
          var href = (node.getAttribute("href") || "").trim();
          if (!href) return;
          // Ignore links that don't actually navigate anywhere useful.
      ...
      var best = firstMatch || firstText || firstReal;
      if (best) firstTested = true;
      return best;
    }
    var tries = 0;
    function attempt() {
      ...
      if (a) {
        ...
        try {
          ...
          catch (e2) { addBubble("jarvis", "Gagal klik link pertama."); }
        }
        if (done) done();
        return;
      }
   ...
   function doClick(target, index) {
    ...
    function doFill(field, value, index) {
     ...
     if (index != null && index !== "") {
       ...
       if (!r.ok && field) r = domFill(field, value);
     } else if (field) {
       ...
       r = { ok: false, reason: "Tiada medan" };
     }
      if (r.ok) addBubble("jarvis", "Mengisi \"" + (field || ("index " + index)) + "\" dengan \"" + value + "\".");
      else addBubble("jarvis", "Gagal isi \"" + (field || index) + "\": " + (r.reason || "medan tak dijumpai") + ". Semak ejaan label medan, atau elemen mungkin dalam iframe lintas-domain / tersembunyi.");
      return r;
   }
  ...
  function domClick(target) {
   ...
   function findInputByField(field) {
     var terms = field.toLowerCase().split(/\s+/).filter(Boolean);
     if (!terms.length) return null;
     // Semantic intent hints: map common words to an expected input type so we
     // pick the RIGHT field (Stagehand-style intent-aware location) instead of
     // the first fuzzy match. e.g. "email" must not fill a password box.
     var fieldText = field.toLowerCase();
     var typeHint = "";
     if (/e-?mail/.test(fieldText)) typeHint = "email";
     else if (/kata\s*laluan|password|kata\s*rahsia/.test(fieldText)) typeHint = "password";
     else if (/(search|cari|query)/.test(fieldText)) typeHint = "search";
     else if (/(telefon|phone|nombor|number)/.test(fieldText)) typeHint = "tel";
     ...
     walkAll(document.documentElement, function (node) {
       ...
       lbl += " " + (inp.id || "") + " " + (inp.getAttribute("aria-label") || "") + " " +
              (inp.getAttribute("placeholder") || "") + " " +
              (inp.getAttribute("title") || "") + " " + (inp.name || "") + " " +
              (inp.getAttribute("type") || "");
       lbl = lbl.toLowerCase();
       var words = lbl.split(/[\s\-_/.:,()\[\]]+/).filter(Boolean);
       var s = 0;
       terms.forEach(function (tt) {
         if (!tt) return;
         if (words.indexOf(tt) !== -1) s += 5;        // whole-word match (robust)
         else if (lbl.indexOf(tt) !== -1) s += 1;     // partial fallback
       });
       // Reward inputs whose type/placeholder matches the asked intent.
       if (typeHint) {
         var typ = (inp.getAttribute("type") || "").toLowerCase();
         var sig = (typ + " " + (inp.getAttribute("placeholder") || "") + " " + (inp.getAttribute("aria-label") || "")).toLowerCase();
         if (sig.indexOf(typeHint) !== -1) s += 4;
         // Penalise clearly-wrong types (e.g. password box for an email ask).
         if (typeHint === "password" && typ === "email") s -= 4;
         if (typeHint === "email" && typ === "password") s -= 4;
       }
   ...
   function isSearchField(inp) {
     try {
       if (!inp) return false;
       if ((inp.getAttribute && inp.getAttribute("type")) === "search") return true;
   ...
   function submitSearchField(inp) {
     ...
     // keydown handler (e.g. a generic search box) performs the search / navigation.
     try {
   ...
   function domFill(field, value) {
     ...
     try {
       ...
       // A search field: the next plan step must run on the results page. For
       // YouTube we return the results URL and let runPlanSequential navigate via
       // the background (reliable pending-plan handoff). For other sites we press
       // Enter and let the resulting navigation be detected on pagehide.
       if (isSearchField(inp)) {
         if (location.hostname && /(^|\.)youtube\.com$/i.test(location.hostname)) {
           return { ok: true, searchNavigate: "https://www.youtube.com/results?search_query=" + encodeURIComponent(value || "") };
         }
  ...
  function getSelectionText() {
    if (SIDEBAR_HOST) return hostSelection || lastPageSelection || "";
    try {
  ...
  function doSummarize() {
    ...
    function doChat(text, onComplete) {
      ...
      withRagDocs(text, ctx, function (ragDocs) {
        ...
        sendToProvider(Core.buildConversationPrompt(text, ctx, history, opts), onComplete);
      });
    }
    ...
    function askSavedLinks(text, onComplete) {
      ...
      api.runtime.sendMessage({ type: "jarvis-library-search", keywords: kws, limit: 20 }).then(function (res) {
        ...
        var header = "Berikut ialah link simpanan pengguna yang PALING RELEKAN dengan soalan (daripada " + total + " link keseluruhan):\n";
    ...
    function reclassifyCategory(text, onComplete) {
      ...
      api.runtime.sendMessage({ type: "jarvis-get-categories" }).then(function (res) {
        ...
        // Kenal pasti kategori SUMBER (B) dan SASARAN (A) daripada teks.
        ...
        if (m) tgtSpan = m[1];
        // Fallback: "kategori X yang tak sesuai" -> X = sumber
        if (!srcSpan) {
        ...
        api.runtime.sendMessage({ type: "jarvis-library-search", categoryId: srcCat.id, limit: 200 }).then(function (r) {
          ...
          sendToProvider(prompt, function (answer) {
            ...
            String(answer || "").split(/\r?\n/).forEach(function (line) {
              ...
              // cari id dengan padanan tepat atau substring
            ...
            // Had batch ~50 (pelan keselamatan).
    ...
    // Temporarily hide the JARVIS panel while a screenshot is being taken so it
    // doesn't appear in the capture, then restore it afterwards. We also suppress
    // the "click outside closes panel" handler during the capture: the click that
    // confirms a region / picks an image lands outside the panel and would
    // otherwise toggle it closed (removing lp-jarvis-open), leaving it hidden for
    // good after we restore display.
    ...
    function ensureCapturePopup() {
      ...
      // Region / image selection draw an overlay on the host document, which in
      // sidebar-host mode is the sidebar page (not the active web page). So there
      // we only offer the full active-tab screenshot (captured by the background).
    ...
    function ensureSelectEls() {
      ...
      // Simpan/Batal bar not shown — region is saved automatically on release
      // (see mouseup handler), so the buttons are never needed.
    ...
    function wireSelectEvents() {
      ...
      window.addEventListener("mouseup", function () {
        var didInteract = regionDrawing || regionMoving || regionResizing;
        regionDrawing = false; regionMoving = false; regionResizing = false;
        // Save the selection immediately on release — no Simpan button needed.
        if (didInteract && regionRect && regionRect.width >= 5 && regionRect.height >= 5) {
          confirmRegion();
        }
      });
      ...
      selectBar.querySelector("#lp-jarvis-select-cancel").addEventListener("click", function (e) {
        e.stopPropagation(); if (_pendingHostCaptureRegionCallback) { _pendingHostCaptureRegionCallback(null); _pendingHostCaptureRegionCallback = null; } cancelRegion(); showPanelAfterCapture();
      });
      document.addEventListener("keydown", function (e) {
        if (e.key === "Escape" && regionMode) { if (_pendingHostCaptureRegionCallback) { _pendingHostCaptureRegionCallback(null); _pendingHostCaptureRegionCallback = null; } cancelRegion(); showPanelAfterCapture(); }
      });
    }
    ...
    function confirmRegion() {
      if (!regionRect || regionRect.width < 5 || regionRect.height < 5) { if (_pendingHostCaptureRegionCallback) { _pendingHostCaptureRegionCallback(null); _pendingHostCaptureRegionCallback = null; } cancelRegion(); showPanelAfterCapture(); return; }
      var rect = regionRect;
      cancelRegion();
      pendingCapturePromise = api.runtime.sendMessage({ type: "jarvis-capture-screenshot" }).then(function (res) {
        ...
        if (!res || !res.ok || !res.dataUrl) {
          ...
          if (_pendingHostCaptureRegionCallback) { _pendingHostCaptureRegionCallback(null); _pendingHostCaptureRegionCallback = null; }
          return null;
        }
        return cropDataUrl(res.dataUrl, rect).then(function (outUrl) {
          ...
          if (_pendingHostCaptureRegionCallback) { _pendingHostCaptureRegionCallback(outUrl); _pendingHostCaptureRegionCallback = null; }
          return outUrl;
        });
      }).catch(function () {
        showPanelAfterCapture();
    ...
    function cancelImageSelect() {
      cleanupImageSelect();
      showPanelAfterCapture();
      if (_pendingHostCaptureImageCallback) { _pendingHostCaptureImageCallback(null); _pendingHostCaptureImageCallback = null; }
    }
    ...
    // Like captureElementByCrop but returns the cropped data URL (does NOT touch
    // pendingImage / the panel) — used by the relayed drag-drop capture so the
    // result can be returned to the sidebar host.
    function captureElementByCropDataUrl(rect) {
      return api.runtime.sendMessage({ type: "jarvis-capture-screenshot" }).then(function (res) {
        if (!res || !res.ok || !res.dataUrl) return null;
        return cropDataUrl(res.dataUrl, rect);
      });
    }
    ...
    function maybeAutoSendImageWithLastQuestion() {
     ...
     function ensureCariImgChip() {
       if (!imgThumb || cariImgChipBtn) return;
     ...
     function showPendingImageThumb(autoSearch) {
       ...
       imgThumb.innerHTML = "";
       cariImgChipBtn = null;
       if (!pendingImage) { imgThumb.style.display = "none"; return; }
    ...
    // Click-to-enlarge any image (bubble thumbnail or attachment) in a simple
    // full-screen lightbox. Reuses the existing image-select/hint z-index band.
    function openImageLightbox(src) {
      if (!src) return;
      var box = el("div", { id: "lp-jarvis-lightbox", className: "lp-jarvis-lightbox" });
      var im = document.createElement("img");
      im.src = src;
      im.className = "lp-jarvis-lightbox-img";
      box.appendChild(im);
      var close = el("div", { className: "lp-jarvis-lightbox-close", text: "✕", title: "Tutup" });
      box.appendChild(close);
      var dismiss = function () { try { if (box.parentNode) box.parentNode.removeChild(box); } catch (e) {} };
      box.addEventListener("click", function (e) { if (e.target === box || e.target === close) dismiss(); });
    ...
    // ---- File upload (feature #6d): let the user pick an image straight from
    // their computer via a hidden <input type="file">, then attach it like a
    // screenshot so the next question can include the picture. ----
  ...
  api.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    ...
    // Drag-and-drop from the sidebar host: capture a web-page image by its URL.
    // Find the source <img> in THIS (active) tab, then crop the visible-tab
    // screenshot to its box — same CORS-free path as "Pilih imej".
    if (!SIDEBAR_HOST && message.type === "jarvis-host-capture-image-by-url") {
      try {
        var _url = message.url;
        var _img = null;
        var _imgs = document.images || [];
        for (var _i = 0; _i < _imgs.length; _i++) {
          var _s = _imgs[_i].currentSrc || _imgs[_i].src || "";
          if (_s === _url) { _img = _imgs[_i]; break; }
        }
        if (!_img) {
          for (var _j = 0; _j < _imgs.length; _j++) {
            var _s2 = _imgs[_j].currentSrc || _imgs[_j].src || "";
            if (_s2 && (_s2.indexOf(_url) === 0 || _url.indexOf(_s2) === 0)) { _img = _imgs[_j]; break; }
          }
        }
        if (_img) {
          var _r = _img.getBoundingClientRect();
          if (_r.width >= 3 && _r.height >= 3) {
            var _rect = { left: _r.left, top: _r.top, width: _r.width, height: _r.height };
            captureElementByCropDataUrl(_rect).then(function (d) {
              try { sendResponse({ ok: !!d, dataUrl: d || null }); } catch (e) {}
            });
            return true;
          }
        }
    ...
    // ── Sidebar-host region capture ────────────────────────────────────────
    // The sidebar JARVIS relays the "Pilih kawasan" action to the active tab
    // so the selection overlay appears on the actual web page, not the sidebar panel.
    if (!SIDEBAR_HOST && message.type === "jarvis-host-capture-region") {
      try {
        // The sidebar content is hidden by the sidebar itself on click (via
        // hidePanelForCapture). We deliberately do NOT call sidebarAction.close()
        // here: in this Firefox build close() only reloads the panel, which would
        // undo that hide. The image is handed back to the reopened sidebar.
        ensureSelectEls();
        regionMode = true; regionRect = null; regionDrawing = false;
        regionMoving = false; regionResizing = false;
        hidePanelForCapture();
        if (selectBox) selectBox.style.display = "none";
        if (selectOverlay) selectOverlay.style.display = "block";
        _pendingHostCaptureRegionCallback = function (dataUrl) {
          _pendingHostCaptureRegionCallback = null;
          cancelRegion();
          showPanelAfterCapture();
          // Stash the (cropped) image for the reloaded sidebar and reopen it. The
          // relay response is fire-and-forget because the sidebar was hidden.
          try { api.runtime.sendMessage({ type: "jarvis-deliver-capture", dataUrl: dataUrl || null }); } catch (eDeliver) {}
          try { sendResponse({ dataUrl: dataUrl || null }); } catch (e) {}
        };
        return true;
      } catch (e) { try { sendResponse(null); } catch (e2) {} return true; }
    }
    // ── Sidebar-host image selection ──────────────────────────────────────
    // The sidebar JARVIS relays the "Pilih imej" action to the active tab.
    if (!SIDEBAR_HOST && message.type === "jarvis-host-capture-image") {
      try {
        // The sidebar hides its own content on click; we don't call close() here
        // for the same reason as region capture above.
        startImageSelect();
        _pendingHostCaptureImageCallback = function (dataUrl) {
          _pendingHostCaptureImageCallback = null;
          // Stash the image for the reloaded sidebar and reopen it.
          try { api.runtime.sendMessage({ type: "jarvis-deliver-capture", dataUrl: dataUrl || null }); } catch (eDeliver) {}
          try { sendResponse({ dataUrl: dataUrl || null }); } catch (e) {}
        };
        return true;
    ...
    if (message.type === "toggle-jarvis-overlay") {
      ...
      if (message.open === true) {
        ...
        if (!open) toggle();
        // Isi & hantar prompt terus bila dibuka melalui "AI button action" = jarvis.
        // Panel dibina secara segerak dalam toggle(), jadi #lp-jarvis-input wujud.
        // Keutamaan: prompt dalam mesej (background) > page variable
        // (floating button) > tiada. Page variable __lpPendingAiPrompt
        // di-set oleh floatingButtonFull.js supaya prompt cuma diproses
        // SELEPAS panel terbuka — elak hantar dua kali.
        var promptToSend = (typeof message.prompt === "string" && message.prompt)
          ? message.prompt
          : (typeof window.__lpPendingAiPrompt === "string" && window.__lpPendingAiPrompt
            ? (function () { var v = window.__lpPendingAiPrompt; window.__lpPendingAiPrompt = null; return v; })()
            : null);
        if (promptToSend) {
          setTimeout(function () {
            try {
    ...
    if (message.type === "jarvis-set-prompt") {
      // Hanya panel SIDEBAR_HOST (jarvisSidebar.html) terima prompt luar — elak
      // overlay pada tab aktif yang tertutup turut isi & hantar (phantom submit).
      if (!SIDEBAR_HOST) return undefined;
      // Diterima dari background bila "AI button action" = jarvis: isikan
      // composer JARVIS dengan teks terpilih dan terus hantar.
      try {
        var _ta = document.getElementById("lp-jarvis-input");
        var _send = document.getElementById("lp-jarvis-send");
  ...
  document.addEventListener("keydown", function (e) {
    ...
    try {
      setTimeout(function () {
        // Skip this check when the panel is intentionally not built on the active
        // tab: (a) JARVIS is suspended while the AI sidebar is open, or (b) this is
        // the sidebar-host instance, whose panel lives in the sidebar page — not
        // here. Firing here would be a false positive (red watchdog banner).
        if (jarvisSuspended || SIDEBAR_HOST) return;
        if (!root || !document.getElementById("lp-jarvis-root")) {
          // Only surface a fatal error if the core script actually failed to load.
          // When Core is present the panel is simply deferred (suspended /
          // sidebar-host), which is expected and not an error.
   ...
   function openWaybackForCurrent() {
     ...
     // Klik "Ya" ialah isyarat pengguna → window.open dibenarkan.
  ...
  // #4 Cross-tab Context Awareness: bila background minta snapshot ringan
  // halaman ini, kumpul tajuk/URL/preview dan hantar balik sebagai
  // `jarvis-context-update` supaya JARVIS tahu apa yang dibuka dalam tab ini.
  function collectJarvisTabContext() {
    var url = "";
    ...
    try {
      ...
      faviconUrl = fl && fl.href ? fl.href : (window.location.origin + "/favicon.ico");
    } catch (e) {}
    return { url: url, title: title, textPreview: preview, faviconUrl: faviconUrl };
  }

  if (api && api.runtime && typeof api.runtime.onMessage !== "undefined") {
    api.runtime.onMessage.addListener(function (message, sender, sendResponse) {
      if (message && message.type === "jarvis-collect-context") {
        try {
          api.runtime.sendMessage({ type: "jarvis-context-update", data: collectJarvisTabContext() })
            .catch(function () {});
  ...
  // #5 Automation Studio — terima tindakan JARVIS yang dimajukan dari background
  // (jadual / tab trigger) dan laksanakannya pada halaman ini.
  function runAutomationAction(action, params) {
    params = params || {};
    try {
      switch (action) {
        case "open_url": doOpenUrl(params.target); break;
        case "navigate": doNavigate(params.target); break;
        case "new_tab": sendBrowser("new_tab", {}); break;
        case "close_tab": sendBrowser("close_tab", {}); break;
        case "close_all_tabs": sendBrowser("close_all_tabs", {}); break;
        case "reload": sendBrowser("reload", {}); break;
        ...
        case "duplicate_tab": sendBrowser("duplicate_tab", {}); break;
        case "bookmark": sendBrowser("bookmark", {}); break;
        case "print_page": try { window.print(); } catch (e) {} break;
        case "zoom": sendBrowser("zoom", { direction: params.direction === "out" ? "out" : "in" }); break;
        case "save": doSave(); break;
        case "summarize": doSummarize(); break;
        case "ask": doChat(params.text || "", function () {}); break;
        case "summarize_selection": doSummarizeSelection(params.query); break;
        case "translate_selection": doTranslateSelection(params.query); break;
        case "copy_url": doCopyUrl(); break;
        case "copy_answer": doCopyAnswer(); break;
        case "click": doClick(params.target, params.index); break;
        case "click_first_link": doClickFirstLink(null); break;
        case "fill": doFill(params.field, params.value, params.index); break;

        case "scroll": doScroll(params.direction === "up" ? "up" : "down"); break;
        default: if (typeof console !== "undefined") console.warn("[Automation] tindakan tak dikenali:", action);
      }
    } catch (e) {}
  }

  if (api && api.runtime && api.runtime.onMessage && api.runtime.onMessage.addListener) {
    api.runtime.onMessage.addListener(function (message, sender, sendResponse) {
      if (message && message.type === "jarvis-automation-action") {
        runAutomationAction(message.action, message.params);
        if (sendResponse) sendResponse({ ok: true });
        return true;
      }
      return false;
